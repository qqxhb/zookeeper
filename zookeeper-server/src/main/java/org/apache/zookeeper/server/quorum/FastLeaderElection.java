...
public class FastLeaderElection implements Election {
			...
			public void run() {
				...
				while (!stop) {
					...
					try {
						...
						// check if we have a version that includes config. If so extract config info
						// from message.
						if (version > 0x1) {
						...
						/*
						 * If it is from a non-voting server (such as an observer or a non-voting
						 * follower), respond right away.
						 */
	...
	private void printNotification(Notification n) {
		LOG.info("Notification: " + Long.toHexString(n.version) + " (message format version), " + n.leader
				+ " (n.leader), 0x" + Long.toHexString(n.zxid) + " (n.zxid), 0x" + Long.toHexString(n.electionEpoch)
				+ " (n.round), " + n.state + " (n.state), " + n.sid + " (n.sid), 0x" + Long.toHexString(n.peerEpoch)
				+ " (n.peerEPoch), " + self.getPeerState() + " (my state)"
	...
	protected boolean totalOrderPredicate(long newId, long newZxid, long newEpoch, long curId, long curZxid,
		 ...
		 * higher 2- New epoch is the same as current epoch, but new zxid is higher 3-
		 * New epoch is the same as current epoch, new zxid is the same as current zxid,
	...
	/**
	 * Check if a given sid is represented in either the current or the next voting
	 * view
	 *
