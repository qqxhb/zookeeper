...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
	...
	/**
	 * This is the secret that we use to generate passwords. For the moment, it's
	 * more of a checksum that's used in reconnection, which carries no security
	 * weight, and is treated internally as if it carries no security weight.
	 */
	...
	/**
	 * Creates a ZooKeeperServer instance. Nothing is setup, use the setX methods to
	 * prepare the instance (eg datadir, datalogdir, ticktime, builder, etc...)
	 *
	 * @throws IOException
	 */
	...
	public ZooKeeperServer(FileTxnSnapLog txnLogFactory, int tickTime, int minSessionTimeout, int maxSessionTimeout,
		...
		LOG.info("Created server with tickTime " + tickTime + " minSessionTimeout " + getMinSessionTimeout()
				+ " maxSessionTimeout " + getMaxSessionTimeout() + " datadir " + txnLogFactory.getDataDir()
	...
	public ZooKeeperServerConf getConf() {
		return new ZooKeeperServerConf(getClientPort(), zkDb.snapLog.getSnapDir().getAbsolutePath(),
				zkDb.snapLog.getDataDir().getAbsolutePath(), getTickTime(),
				serverCnxnFactory.getMaxClientCnxnsPerHost(), getMinSessionTimeout(), getMaxSessionTimeout(),
				getServerId());
	}

	/**
	 * This constructor is for backward compatibility with the existing unit test
	 * code. It defaults to FileLogProvider persistence provider.
	 */
	...
	public void loadData() throws IOException, InterruptedException {
		/*
		 * When a new leader starts executing Leader#lead, it invokes this method. The
		 * database, however, has been initialized before running leader election so
		 * that the server could pick its zxid for its initial vote. It does it by
		 * invoking QuorumPeer#getLastLoggedZxid. Consequently, we don't need to
		 * initialize it once more and avoid the penalty of loading it a second time.
		 * Not reloading it is particularly important for applications that host a large
		 * database.
		 * 
		 * The following if block checks whether the database has been initialized or
		 * not. Note that this method is invoked by at least one other method:
	...
	public void expire(Session session) {
		...
		LOG.info("Expiring session 0x" + Long.toHexString(sessionId) + ", timeout of " + session.getTimeout()
				+ "ms exceeded");
		close(sessionId);
	}
	...
	/**
	 * Change the server ID used by {@link #createSessionTracker()}. Must be called
	 * prior to {@link #startup()} being called
	 *
	...
	/**
	 * Sets the state of ZooKeeper server. After changing the state, it notifies the
	 * server state change to a registered shutdown handler, if any.
	 * <p>
	 ...
	 * disk.</li>
	 * <li>During shutdown the server sets the state to SHUTDOWN, which corresponds
	 * to the server not running.</li>
	 *
	...
	/**
	 * This can be used while shutting down the server to see whether the server is
	 * already shutdown or not.
	 *
	 ...
	 * 
	 * @param fullyShutDown true if another server using the same database will not
	 *                      replace this one in the same process
	 */
	...
	/**
	 * This structure is used to facilitate information sharing between PrepRP and
	 * FinalRP.
	 */
	...
	public void reopenSession(ServerCnxn cnxn, long sessionId, byte[] passwd, int sessionTimeout) throws IOException {
		if (checkPasswd(sessionId, passwd)) {
			...
			LOG.warn("Incorrect password from " + cnxn.getRemoteSocketAddress() + " for session 0x"
	...
	public void finishSessionInit(ServerCnxn cnxn, boolean valid) {
		...
		try {
			...
			if (valid) {
				LOG.debug("Established session 0x" + Long.toHexString(cnxn.getSessionId()) + " with negotiated timeout "
						+ cnxn.getSessionTimeout() + " for client " + cnxn.getRemoteSocketAddress());
				cnxn.enableRecv();
			} else {

				LOG.info("Invalid session 0x" + Long.toHexString(cnxn.getSessionId()) + " for client "
						+ cnxn.getRemoteSocketAddress() + ", probably expired");
	...
	/**
	 * If the underlying Zookeeper server support local session, this method will
	 * set a isLocalSession to true if a request is associated with a local session.
	 *
	 * @param si
	 */
	...
	private Record processSasl(ByteBuffer incomingBuffer, ServerCnxn cnxn) throws IOException {
		...
		try {
			...
			try {
				...
				// if using the DIGEST-MD5 mechanism, clientToken will be empty at the beginning
				// of the
	 ...
	 * {@link ZooKeeperServerShutdownHandler#handle(State)} will be called for every
	 * server state changes {@link #setState(State)}.
	 *
